...
np.save("data/X-mfcc-test", X)
np.save("data/y-mfcc-test", y)

le = LabelEncoder()
y_test_encoded = to_categorical(le.fit_transform(y))
y_encoded = pd.DataFrame(y_test_encoded)
y_encoded.to_csv('y_test.csv', index=False, index_label=False, header=False)

X_reshape = X.reshape(len(padded_features), 30*32)
X = pd.DataFrame(X_reshape)
X.to_csv('X_test.csv', index=False, index_label=False, header=False)

with open('parameter-test.pickle', 'wb') as handle:
